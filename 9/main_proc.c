...
void try_unlink_log_queue() {
    mqd_t test = mq_open(LOG_QUEUE, O_RDONLY);

    if (test == (mqd_t)-1) return;
    mq_close(test);
    mq_unlink(LOG_QUEUE);
}
...
void *gardener_thread(void *arg) {
    ...
    while (shm && shm->system_running) {
        ...
        if (shm->flowers[id].st == NEED_WATER) {
            ...
            log_msg("[gardener %ld] watering flower %d\n", gardener_id, id);
            sleep(shm->flowers[id].time_to_water);
            printf("[gardener %ld] finished watering flower %d\n", gardener_id, id);
            log_msg("[gardener %ld] finished watering flower %d\n", gardener_id, id);
        } else {
            shm->flowers[id].in_queue = 0;
        }
    ...
    log_msg("[Gardener %ld] exiting\n", gardener_id);
    return NULL;
}

void cleanup() {
    printf("[MAIN] Cleanup started\n");
    log_msg("[MAIN] Cleanup started\n");
    ...
    log_msg("[MAIN] Cleanup completed\n");

    try_unlink_log_queue();
}
...
int main() {
    ...
    log_msg("Ожидаю запуска всех %d процессов-цветов...\n", NUM_PROCS);
	...
	while (1) {
		...
		if (count == NUM_PROCS) {
            ...
            log_msg("Все %d процессов запущены!\n", NUM_PROCS);
			break;
		}
		printf("Запущено %d/%d процессов. Ожидаю...\n", count, NUM_PROCS);
        log_msg("Запущено %d/%d процессов. Ожидаю...\n", count, NUM_PROCS);
		sleep(1);
	}
    ...
    log_msg("[MAIN] System started. Press Ctrl+C to stop or wait, when all flowers will die.\n");

    while (shm->system_running) {
		printf("[MAIN] this much flowers are alive now: %d\n", shm->alive_flowers);
        log_msg("[MAIN] this much flowers are alive now: %d\n", shm->alive_flowers);
